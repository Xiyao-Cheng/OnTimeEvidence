...
def query(s="neuron simulation"):
	...
	ix = index.create_in("C:\\Users\\vekar\\Desktop\\OnTimeRecommend\\JupyterNotebook\\notebooks\\", schema)

	# Get a writer form the created index in 
	writer = ix.writer()
	output = []
	walkFolder(writer,"C:\\Users\\vekar\\Desktop\\OnTimeRecommend\\JupyterNotebook\\notebooks\\")
	# Commit changes
	...
	with ix.searcher(weighting=scoring.TF_IDF()) as s:
		results = s.search(q)
		for hit in results:
			fileNamelst = hit['filename'].split('\\')
			size = len(fileNamelst)-1
			#print("Cell {} of Notebook '{}'".format(hit['cell_no'],fileNamelst[size]))
			output.append({"cell_no": hit['cell_no'], "filename": fileNamelst[size]})
...
def main():
	...
	with ix.searcher(weighting=scoring.TF_IDF()) as s:
		...
		for hit in results:
			...
			output.append({"cell_no": hit['cell_no'], "filename": hit['filename']})
