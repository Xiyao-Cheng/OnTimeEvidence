...
app.controller('survay-controller',function($scope, $window ,$rootScope,$location,mainService){
	...
	$scope.submitSurvay = function(){
		...
		mainService.callPostRestAPI("/chatbot/submitSurvay/", input).then(function (data) {
			alert("Thanks for submitting survey");
			$window.location.href = 'http://localhost:9100/CyNeuro/#!/';
		});
	}


});
app.controller('survay-controller-bio',function($scope, $window ,$rootScope,$location,mainService){
