...
$(document).ready(function () {
    ...
    //OnTimeRecommend Client SDK changes : chnage survey url here : hostname, port 
    var survey_url = "http://localhost:9100/CyNeuro/getSurvay"; 
    
    //OnTimeRecommend Client SDK changes : chnage survey url here : hostname, port for checkForuser profile service
    var check_user_profile = "http://localhost:8050/user-profile-services/checkForUser";
    
    //OnTimeRecommend Client SDK changes : chnage survey url here : hostname, port for chain recommend service
    var chain_rec = "http://localhost:8050/chain-recommender-services/getRecommendationSuggestion";
    var domain = "neuro";
        ...
        $.ajax({
            ...
            success: function (data, status, jqXHR) {
            	...
            	if(undefined ==  data['userQuadrant'] || data['userQuadrant'] == ''){
            		...
            		<a href= "' + survey_url + '?id='+data['id'] +'"  style="color:red"> Click Here to take survey </a> ');
            	}else{
            		...
            		user_profile = data['userQuadrant'] ;
            		//OnTimeRecommend Client SDK changes: Change access token here (developer access token from google dialogueflow). 
            ...
            error: function (jqXHR, status) {
                // error handler
                console.log(jqXHR);
                alert('Please contact system administartor.');
            }
         });
    ...
    function main(data) {
        ...
        if(data.result.parameters){
        	if(data.result.parameters.rec_id){
    			 ...
    			 var chain_rec = data.result.parameters.chain_rec;
    			 if(any && any != ''){
    			 ...
    			 }else if(chain_rec && chain_rec == 'true'){
    				 callBotRes = false;
    				 chainRecommender(rec_id, intentName, speech);	
    			 }
        	}
        }
    ...
    function chainRecommender(rec_id, intentName, speech){
    	
    	data = {};
        data['userProfile']=user_profile;
        data['clientId']=domain;
        data['recKey']=rec_id;
        data['intentId']=intentName;
    	...
    	$.ajax({
             ...
             success: function (data, status, jqXHR) {
            	suggestions = JSON.parse(data.suggestions);
             	...
             	var speeach_temp = "According to me you should first, go through ";
             	
             	for(i in suggestions){
             		speeach_temp =  speeach_temp + '<a href="javascript:getRecommendation(\'' + rec_id + '\',\'' + suggestions[i] + '\')" >'+suggestions[i]+'</a>, ';
             	}

             	speech = speeach_temp + speech;
             	setBotResponse(speech);
             },
             error: function (jqXHR, status) {
                 // error handler
